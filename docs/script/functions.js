...
const catOrder = ['trig', 'numeric', 'bitwise', 'stats', 'matrix', 'symbolic', 'calc', 'string', 'type', 'special'];
...
function render() {
  ...
  let filtered = functions.filter(f => {
    ...
    if (q) {
      return f.name.toLowerCase().includes(q) || f.desc.toLowerCase().includes(q) || f.cat.includes(q);
    }
    return true;
  });
